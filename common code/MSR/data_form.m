function [hugeC, highc, consD, D] = data_form(n_thermal, n_wind, demand, T, TLC, PTDF, ...,
...
%(3.5) and (3.7) ramping constraints
J = [eye(n_thermal-1), zeros(n_thermal - 1, 2 * n_wind); zeros(n_wind, n-1), eye(n_wind)];
tempC1 = zeros(2 * (n-1) * (T-1), N * T);
for i = 1:T-1
    row_start = (i-1)*(n-1) + 1;
    row_end = i * (n-1);
    column_start = (i-1)*N+1;
    ...
    row_start = row_start + (n-1)*(T-1);
    row_end = row_end + (n-1)*(T-1);
    tempC1(row_start:row_end, column_start:column_end) = [J, -J];
end
...
tempC2 = [J, zeros(n-1, N * (T - 1))];
tempc2 = g_0(2:end) + ramp_up(2:end);
tempC3 = [-J, zeros(n-1, N * (T - 1))];
